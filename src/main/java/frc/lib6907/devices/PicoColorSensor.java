...
public class PicoColorSensor implements AutoCloseable {
  ...
  private ColorMatch mColorMatcher1 = new ColorMatch();
  private ColorMatch mColorMatcher2 = new ColorMatch();

  private Color red = new Color(0.5, 0.16, 0.34);
  ...
  public Alliance getMatchedColor(Color rawColor) {
    try {
      // if (prox < 140) {
      //   return Alliance.Invalid;
      // }
      ColorMatchResult result;// = colorMatcher.matchClosestColor(rawColor);
      result = mColorMatcher1.matchClosestColor(rawColor);
      ...
      if (result.color.equals(blue)) {
        ...
        System.out.println("color 1 match to: " + result.color.red + " " + result.color.green + " " + result.color.blue);
        return Alliance.Invalid;
      }
  ...
  public void initializeColorMatch(Color red, Color blue) {
    this.red = red;
    this.blue = blue;
    mColorMatcher1.addColorMatch(red);
    mColorMatcher1.addColorMatch(blue);
  }
  ...
  public Color getColor0() {
    try {
      threadLock.lock();
      double sum = color0.red + color0.green + color0.blue;
      return new Color(color0.red/sum, color0.green/sum, color0.blue/sum);
    } finally {
      threadLock.unlock();
    }
  }
  ...
  public Color getColor1() {
    try {
      threadLock.lock();
      double sum = color1.red + color1.green + color1.blue;
      return new Color(color1.red/sum, color1.green/sum, color1.blue/sum);
    } finally {
      threadLock.unlock();
    }
  }
