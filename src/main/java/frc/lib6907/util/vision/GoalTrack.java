...
public class GoalTrack {
    ...
    // proj distance between frames that are considered the same goaltrack (m)
    private static final double kMaxTrackerDistance = 0.5;
    // observation window size (sec)
    private static final double kMaxGoalTrackAge = 2.5;
    // smooth pos filter window size (sec)
    private static final double kMaxGoalTrackSmoothingTime = 0.5;
    // (fps)
    private static final double kCameraFrameRate = 22.0;

    TreeMap<Double, GPose2d> mObservedPositions = new TreeMap<>();
    GPose2d mSmoothedPosition = null;
    int mId;
    ...
    public static synchronized GoalTrack makeNewTrack(double timestamp, Pose2d first_observation, int id) {
        ...
        GPose2d gp2d = new GPose2d(first_observation);
        rv.mObservedPositions.put(timestamp, gp2d);
        rv.mSmoothedPosition = gp2d;
        rv.mId = id;
        return rv;
    }
     ...
     */
    public synchronized boolean tryUpdate(double timestamp, TargetInfo new_observation) {
        if (!isAlive()) {
            return false;
        }
        double distance = mSmoothedPosition.inverse().transformBy(new_observation.getPose2d()).getTranslation().getNorm();
        if (distance < kMaxTrackerDistance) {
            mObservedPositions.put(timestamp, new GPose2d(new_observation.getPose2d()));
            pruneByTime();
            return true;
        } else {
            emptyUpdate();
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            int num_samples = 0;
            for (Map.Entry<Double, GPose2d> entry : mObservedPositions.entrySet()) {
                if (t_now - entry.getKey() > kMaxGoalTrackSmoothingTime) {
                    continue;
                }
                ++num_samples;
            ...
            if (num_samples == 0) {
                ...
                mSmoothedPosition = new GPose2d(x, y, new GRotation2d(c, s));
            }
        }
    }

    public synchronized GPose2d getSmoothedPosition() {
        return mSmoothedPosition;
    }

    public synchronized GPose2d getLatestPosition() {
