...
public class GoalTracker {
    ...
    public static class TrackReport {
        ...
        public GPose2d field_to_target;
    ...
    public synchronized void update(double timestamp, List<TargetInfo> field_to_goals) {
        // Try to update existing tracks
        for (TargetInfo target : field_to_goals) {
            ...
            if (!hasUpdatedTrack) {
                ...
                mCurrentTracks.add(GoalTrack.makeNewTrack(timestamp, target.getPose2d(), target.id));
            }
        }
        maybePruneTracks();
    }
